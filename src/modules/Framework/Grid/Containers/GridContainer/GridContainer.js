...
export class GridContainer extends Component {
  ...
  render() {
    const {source = [], columns, loading, selectable=true, pagination=false} = this.props;
    const {rowSelection, defaultPageSize = 4} = this.props;
    let gridColumns = [...columns];

    if (selectable) {
      const selectCol = {
        header: (props) => <SelectRow row={{id: 'all'}} onChange={this.selectAllRows} checked={this.isAllRowsSelected()}/>,
        accessor: 'selected',
        width: 40,
        render: ({row}) => <SelectRow row={row} onChange={this.rowSelect} checked={rowSelection(row.id)}/>
      };
      ...
      <div className="grid">
        <ReactTable
          ...
          getTrProps={(state, rowInfo, column, instance) => {
            const isSelected = (!rowInfo || !rowInfo.row) ? false : rowSelection(rowInfo.row.id);
