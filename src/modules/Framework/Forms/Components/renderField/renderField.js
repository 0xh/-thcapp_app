...
export const renderField = ({
  ...
  fieldWrapperClasses = '', labelWrapperClasses = '', inputWrapperClasses = '',
  customChecked = null, options, selectedOption = ''
}) => {
  ...
  return (
    <div className={`field-container ${fieldWrapperClasses}`}>
      <div className={`field-label ${labelWrapperClasses}`}>{label}&nbsp;{required ?
        <span className="required">*</span> : null}</div>
      <div className={`field-input ${inputWrapperClasses}`}>
        <Inputs type={type} input={input} extra={extra} />

        <p className="error-message">{touched && ((error && <span>{error}</span>) || (warning &&
        <span>{warning}</span>))}</p>
      </div>
    </div>
  );
