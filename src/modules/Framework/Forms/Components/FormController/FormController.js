...
const FormController = ({
                          name, children, handleSubmit, handleReset, submitSucceeded, submitting, pristine, invalid, submitValue, resetValue = 'Clear',
                          btnWrapperClasses = 'col-xs-12 col-sm-12',
                          showButtons = true, showReset = true, customError,
                          values = {},
  ...
  <form className={`form ${name}-form`} onSubmit={handleSubmit} autoComplete="false">
    ...
    {customError ?
      <div className="field-container">
        <div className="alert alert-danger" role="alert">{customError}</div>
      </div>
      : null}
      ...
      <div className="field-container">
        <div className={`${btnWrapperClasses}`}>
          <div className={classNames({
            ...
            <button className="button submit" type="submit" disabled={(pristine || invalid) || submitting}>
              {submitValue}
            </button>
            &nbsp;

            {showReset ?
              <button type="button" className="button default" disabled={pristine || submitting}
                      onClick={handleReset}>{resetValue}</button>
              : null
            }
          </div>
        </div>
